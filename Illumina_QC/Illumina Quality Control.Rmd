...
<!--chapter:end:04-Poorer_quality_data.Rmd-->
...
```{r, echo = FALSE}
opts_p <- c("__15-19__", "__235-239__", answer="__251__")
```
...
```{r, echo = FALSE}
opts_p <- c(answer="__15-19__", "__235-239__", "__251__")
```
...
```{r, echo = FALSE}
opts_p <- c("__15-19__", answer="__235-239__", "__251__")
```
6.  At what base position/s does the IQR (Interquartile Range/yellow box) drop below a quality of Q28 for the R2 data?`r longmcq(opts_p)`

```{r, echo = FALSE}
opts_p <- c(answer="__37__", "__129__", "__250-252__")
```
...
```{r, echo = FALSE}
opts_p <- c("__37__", answer="__129__", "__250-252__")
```
8. What is the total count of overrepresented sequences in R1? `r longmcq(opts_p)`

```{r, echo = FALSE}
opts_p <- c("__37__", "__129__", answer="__250-252__")
```
...
```
4. In R1, do the IQRs and/or whiskers drop below Q28 in the "Per base sequence quality" graph? `r longmcq(opts_p)`

```{r, echo = FALSE}
opts_p <- c(answer="__Yes__", "__No__")
```
5. In R2, do the IQRs and/or whiskers drop below Q28 in the "Per base sequence quality" graph? `r longmcq(opts_p)`

```{r, echo = FALSE}
opts_p <- c("__Yes__", answer="__No__")
```
6. In R2, do the IQRs drop below Q28 in the "Per base sequence quality" graph? `r longmcq(opts_p)`
...
<!--chapter:end:05-Good_quality_data.Rmd-->
...
The read data is within your `~/QC_workshop/Illumina_Q/exercise_1` directory (ensure to change directory to this). The reads have been subsampled so you will be working on 100,000 paired reads.
